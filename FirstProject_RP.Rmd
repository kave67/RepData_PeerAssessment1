...
activity <- read.csv("C:\\Users\\kav\\OneDrive\\Documents\\activity.csv", stringsAsFactors=FALSE)
str(activity)
summary(activity)
```
...
The most popular methods are Constant value, Mean/mode/Median and other depending on the underying distribution. 
...
Imputing missing values, (average of the number of steps taken each day is increased, wheras median is decreased). By Imputing missing value, one could notice that the total daily number of steps is increased.
